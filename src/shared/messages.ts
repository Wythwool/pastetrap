import type { AppSettings, LatestTabState, LogEntry, ScanResult, TrustScope } from '@/shared/types';

export interface ScanDoneResponse {
  ok: true;
  scanId: string | null;
  updatedAt: string | null;
  fingerprint: string | null;
  level: ScanResult['level'] | null;
}
...
export type RuntimeMessage =
  ...
  | { type: 'PT_TRUST_SITE'; url: string; scope: TrustScope; includeSubdomains?: boolean }
  | { type: 'PT_UNTRUST_DOMAIN'; domain: string }
  | { type: 'PT_UNTRUST_SITE'; ruleId: string }
  | { type: 'PT_UNTRUST_SITE_FOR_URL'; url: string }
...
export type RuntimeResponseMap = {
  ...
  PT_TRUST_SITE: { settings: AppSettings };
  PT_UNTRUST_DOMAIN: OkResponse;
  PT_UNTRUST_SITE: { settings: AppSettings };
  PT_UNTRUST_SITE_FOR_URL: { settings: AppSettings };
  ...
  PT_RESCAN_TAB: OkResponse & { state: LatestTabState | null; scan: ScanDoneResponse | null };
  PT_GET_SESSION_IGNORES: { sessionIgnores: string[] };
  PT_TRIGGER_SCAN: ScanDoneResponse;
};
