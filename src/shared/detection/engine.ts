...
import { suppressRuleMatchesUrl } from '@/shared/utils/domain';
...
export function analyzePage(snapshot: PageSnapshot, settings: AppSettings, source: ScanSource): ScanResult {
  ...
  const combinedMatches = [...pageMatches, ...commandMatches]
    .filter((match) => !settings.suppressRules.some((rule) => suppressRuleMatchesUrl(rule, snapshot.url, match.category)))
  ...
  const fingerprintSeed = [
    snapshot.url,
    level,
    ...combinedMatches.map((match) => match.id),
    ...commands.map((command) => command.fingerprint)
  ].join('|');
