...
export function scoreMatches(matches: DetectionRuleMatch[], settings: AppSettings): {
  ...
  const highSignalCount = positiveMatches.filter((match) => (ruleCatalog[match.id]?.weight ?? 0) >= 16).length;
  ...
  if (
    ruleIds.has('clipboard-payload-command') &&
    (ruleIds.has('run-dialog-instruction') || ruleIds.has('fake-verification-language') || ruleIds.has('clipboard-bait-language'))
  ) {
    score += 10;
    pushTrace(trace, 'compound-clipboard-payload-flow', 10, 'clipboard-payload-plus-social-engineering');
  }

  if (ruleIds.has('shadow-dom-bait') && (ruleIds.has('clipboard-bait-language') || ruleIds.has('remote-download-execute'))) {
    score += 6;
    pushTrace(trace, 'compound-shadow-bait', 6, 'shadow-dom-plus-bait');
  }
